...
def get_resources_from_online(parent_resource, child_resource, country_prefix=None):
    ...
    while data_to_query:
        page += 1
        url_resource = SERVER_URL + parent_resource + ".json?fields=id,name,"+child_resource+"&filter=programType:eq:WITHOUT_REGISTRATION&pageSize=" + str(PAGESIZE) + "&format=json&order=created:ASC&skipMeta=true&page=" + str(page)
