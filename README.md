...
This script checks if a program rule variable has not "and", "or" or "not" in its name. Also checks if the PRV name contains unexpected characters. And also checks if there are PRV names duplicated for the same program (including case insensitive and trimmed).
